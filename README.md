...
[English overview](README.en.md) | 中文说明如下

## English overview

This repository is an open-source **LaTeX-to-DOCX conversion toolkit for Chinese academic thesis formatting**. It uses Pandoc, a Lua filter, a generated Word reference template, and Python-based OOXML post-processing to convert LaTeX thesis sources into editable Word/WPS `.docx` drafts that follow university thesis-layout requirements.

Although the initial implementation targets Tianjin University of Finance and Economics (TJUFE) management graduate thesis rules, the underlying problem is broader: many Chinese universities require final thesis submissions in institution-compliant Word documents, while researchers often prefer writing in LaTeX. This project bridges that gap with a reproducible, auditable toolchain rather than manual reformatting.

Key technical components:

- `pandoc` for LaTeX parsing and DOCX generation.
- `filters/tjufe-thesis.lua` for thesis-structure normalization during Pandoc conversion.
- `reference.docx` generated by `scripts/build_reference_docx.py` as the Word style baseline.
- `scripts/postprocess_tjufe_docx.py` for OOXML-level fixes such as sections, page numbers, headers/footers, captions, equations, cross-references, footnotes, bibliography layout, and appendix numbering.
- Public sample files and clause-by-clause implementation notes for reproducibility and review.

For maintainers and reviewers: this is a real open-source document-engineering project, not merely a Chinese-language template. The Chinese README remains the primary user guide because the target formatting rules and end users are Chinese, while this English overview is provided to make the project scope and engineering value easier to evaluate internationally.
